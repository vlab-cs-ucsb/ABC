...
class StringConstraintSolverOld: public AstTraverser {
  using TermValueMap = std::map<SMT::Term_ptr, Value_ptr>; // holds multitrack
public:
  StringConstraintSolverOld(SMT::Script_ptr, SymbolTable_ptr, ConstraintInformation_ptr);
  virtual ~StringConstraintSolverOld();
