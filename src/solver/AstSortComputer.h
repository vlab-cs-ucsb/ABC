...
#ifndef SOLVER_TERMTYPECOMPUTER_H
#define SOLVER_TERMTYPECOMPUTER_H
...
class AstSortComputer: public SMT::Visitor {
public:
  AstSortComputer(SMT::Script_ptr, SymbolTable_ptr);
  virtual ~AstSortComputer();

  void start() override;
  void start(SMT::Visitable_ptr term);
  void end() override;
  ...
  SMT::Sort::Type GetTermSort(SMT::Visitable_ptr term);
  bool IsBoolTerm(SMT::Term_ptr term);

private:

  void SetTermSort(SMT::Visitable_ptr term, SMT::Sort::Type sort_type);
  std::map<SMT::Visitable_ptr, SMT::Sort::Type> term_sort_map_;
...
#endif //SOLVER_TERMTYPECOMPUTER_H
