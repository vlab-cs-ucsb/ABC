...
class ConstantTermOptimization: public SMT::Visitor {
 public:
  enum class Mode : int {PREFIX, SUFFIX}; //mode determines whether to remove a prefix or suffix. 

  ConstantTermOptimization();
  virtual ~ConstantTermOptimization();

  void start(SMT::Term_ptr term, int length, ConstantTermOptimization::Mode mode = ConstantTermOptimization::Mode::PREFIX);
  void start(SMT::TermConstant_ptr term_constant, int length, ConstantTermOptimization::Mode mode = ConstantTermOptimization::Mode::PREFIX);
  void end() override;
 ...
 protected:
  ConstantTermOptimization::Mode mode_;
  int length_; //Length of substring to be removed. 
...
#endif /* SRC_SOLVER_OPTIMIZATION_ConstantTermOptimization_H_ */
