...
void CachingConstraintSolver::visitAssert(Assert_ptr assert_command) {
  ...
  if (has_cached_result) {
    ...
    LOG(INFO) << "DONE READING DATA, RETURNING";
    return;
  } else {
  ...
  /* BEGIN main constraint solving algorithm */
  ...
  /* END main constraint solving algorithm */
...
void CachingConstraintSolver::visitAnd(And_ptr and_term) {
  ...
  if(Option::Solver::SUB_FORMULA_CACHING) {
    ...
    // Preorder traversal to identify previously cached subformulas
    // If found and is unsat, shortcircuit & return false
    bool has_cached_result = false;
    ...
    // what does this mean if and_term has no children (has_cached_result = false)?
