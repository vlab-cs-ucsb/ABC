...
SetLogic::SetLogic(Primitive_ptr symbol)
    : Command(Command::Type::SET_LOGIC),
      symbol(symbol) {
}
...
DeclareFun::DeclareFun(Primitive_ptr symbol, SortList_ptr sort_list, Sort_ptr sort)
    : Command(Command::Type::DECLARE_FUN),
      symbol(symbol),
      sort_list(sort_list),
      sort(sort) {
}
...
CheckSat::CheckSat()
    : Command(Command::Type::CHECK_SAT),
      symbol(nullptr) {
}
...
CheckSatAndCount::CheckSatAndCount(Primitive_ptr bound)
    : Command(Command::Type::CHECK_SAT_AND_COUNT),
      bound(bound),
      symbol(nullptr) {
}
...
Exists::Exists(SortedVarList_ptr sorted_var_list, Term_ptr term)
    : Term(Term::Type::EXISTS),
      sorted_var_list(sorted_var_list),
      term(term) {
}
...
Let::Let(VarBindingList_ptr var_binding_list, Term_ptr term)
    : Term(Term::Type::LET),
      var_binding_list(var_binding_list),
      term(term) {
}
...
AsQualIdentifier::AsQualIdentifier(Identifier_ptr identifier, Sort_ptr sort)
    : Term(Term::Type::ASQUALIDENTIFIER),
