...
//  std::cin.get();
  ...
  LOG(INFO) << "Num exceptions = " << number_of_exceptions;
  LOG(INFO) << "Num variables = " << number_of_variables;
  LOG(INFO) << 1;
  ...
  if(first_if || second_if) {
    LOG(INFO) << "first if || second if = true";
    statuses[0] = '+';
  }
  LOG(INFO) << 5;
  //state 0
...
//            LOG(INFO) << "Found int: " << str_value << " = " << std::stoi(str_value);
