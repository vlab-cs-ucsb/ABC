...
namespace Vlab
{
  namespace Theory
  {
    namespace Libs
    {
      ...
      bool MONALib::DFAIsMinimizedOnlyAcceptingEmptyInput(const MONALib::DFA_ptr minimized_dfa) {
        ...
        for (int s = 0; s < minimized_dfa->ns; s++) {
          if (MONALib::DFAIsAcceptingState(minimized_dfa, s) and s != minimized_dfa->s) {
            return false;
          } else if (MONALib::DFAIsOneStepAway(minimized_dfa, s, minimized_dfa->s)) { // if initial state is reachable in a minimized auto, there is a loop
            return false;
          }
        }
        return true;
      ...
      MONALib::DFA_ptr MONALib::DFAProjectAwayAndReMap(const MONALib::DFA_ptr dfa, const int number_of_bdd_variables, const int index) {
        MONALib::DFA_ptr projected_dfa = dfaProject(dfa, (unsigned)index);
      ...
      MONALib::DFA_ptr MONALib::DFAProjectTo(const MONALib::DFA_ptr dfa, const int number_of_bdd_variables, const int index) {
      ...
      MONALib::DFA_ptr MONALib::DFAMakeAcceptingAnyWithInRange(const int start, const int end, const int number_of_bdd_variables) {
      ...
      MONALib::DFA_ptr MONALib::DFAMakeAcceptingAnyAfterLength(const int length, const int number_of_bdd_variables) {
        CHECK(length >= 0);
        // 1 initial state
      ...
      std::unordered_map<std::string, int> MONALib::DFAGetTransitionsFrom(const MONALib::DFA_ptr dfa, const int from, const int number_of_bdd_variables, std::string extra_bits) {
      ...
      std::unordered_set<std::string> MONALib::DFAGetTransitionsFromTo(const MONALib::DFA_ptr dfa, const int from, const int to, const int number_of_bdd_variables, std::string extra_bits) {
        const int* bdd_indices = MONALib::GetBddVariableIndices(number_of_bdd_variables);
      ...
      MONALib::DFA_ptr MONALib::DFAConcat(const MONALib::DFA_ptr dfa1, const MONALib::DFA_ptr dfa2, const int number_of_bdd_variables) {
        ...
        if (left_hand_side_accepts_emtpy_input or right_hand_side_accepts_empty_input) {
          auto any_input_other_than_empty = MONALib::DFAMakeAcceptingAnyAfterLength(1, number_of_bdd_variables);
